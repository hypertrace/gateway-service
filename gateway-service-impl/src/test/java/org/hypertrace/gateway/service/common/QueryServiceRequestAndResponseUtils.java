...
public class QueryServiceRequestAndResponseUtils {
  /**
   * resultsTable contains a String 2 dimensional array aka a table. We are converting that table to
   * the format the query-service will send back. query-service is expected to return String types
   * but this can be modified to handle other types.
   *
   * @param columnNames
   * @param resultsTable
   * @return
  ...
  public static Expression createQsAggregationExpression(
      String functionName, String functionAlias, String columnName, String columnAlias) {
  ...
  public static Expression createQsAggregationExpression(
      String functionName, String columnName, String alias) {
    return Expression.newBuilder()
        .setFunction(
            Function.newBuilder()
                .setFunctionName(functionName)
  ...
  public static OrderByExpression createQsOrderBy(Expression expression, SortOrder sortOrder) {
    return OrderByExpression.newBuilder().setOrder(sortOrder).setExpression(expression).build();
  }

  public static Filter createQsRequestFilter(
  ...
  public static Filter createQsDefaultRequestFilter(
