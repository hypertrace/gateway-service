...
public class TimeRangeFilterUtil {
  public static Filter addTimeRangeFilter(
      String timestampAttribute, Filter filter, long startTime, long endTime) {
    ...
    filterBuilder
        .addChildFilter(getTimestampFilter(timestampAttribute, Operator.GE, startTime))
            ...
            Expression.newBuilder()
                .setLiteral(
                    LiteralConstant.newBuilder()
                        .setValue(
                            Value.newBuilder().setValueType(ValueType.LONG).setLong(timestamp))))
        .build();
  }
}
