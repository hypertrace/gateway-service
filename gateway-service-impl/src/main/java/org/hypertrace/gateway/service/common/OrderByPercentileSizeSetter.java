...
/**
 * Set the size for percentiles in order by if it is not set. This is to give UI the time to fix the
 * bug which does not set the size when they have order by in the request. Will delete this once the
 * bug has been fixed in the UI.
 */
public class OrderByPercentileSizeSetter {
  ...
  private static List<OrderByExpression> addArgumentsToOrderByPercentile(
      EntitiesRequest originalRequest) {
    Expression percentileArgument =
        originalRequest.getSelectionList().stream()
            .filter(
                selection ->
                    selection.hasFunction()
                        && selection.getFunction().getFunction() == FunctionType.PERCENTILE)
    ...
    return originalRequest.getOrderByList().stream()
        .map(
            orderByExpression -> {
              ...
              if (expression.hasFunction()
                  && expression.getFunction().getFunction() == FunctionType.PERCENTILE
                  && expression.getFunction().getArgumentsList().stream()
                      .filter(Expression::hasLiteral)
                      .findFirst()
                      .isEmpty()) {
